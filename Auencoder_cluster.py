...
def loadCSVfile(path,flag="\t"):
    import numpy as np
    tmp = np.loadtxt(path, dtype=np.float, delimiter=flag)

    data = tmp[:,:30].astype(np.float)#加载数据部分
    label = tmp[:,0].astype(np.float)#加载类别标签部分
    return data, label #返回array类型的数据


def read_iris_data(similarity):
    
    data,labels=loadCSVfile('./iris.csv',flag=',')
    print(data)
    print(np.shape(data))
...
data,label=trainSet_correlation.whole_dataset()
